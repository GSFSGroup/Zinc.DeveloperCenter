...
namespace RedLine.UnitTests.Application.A3.Authorization.AuthorizationPolicyTests
{
    public class IsAuthorizedShould : AuthorizationTestBase
    {
        ...
        [Theory]
        [InlineData("GSFSGroup:Activity:GenericActivity")] // explicit activity grant
        [InlineData("*:Activity:GenericActivity")] // explicit activity grant, wildcard tenant
        [InlineData("GSFSGroup:*:GenericActivity")] // bizzare, but valid wildcard grant type
        [InlineData("GSFSGroup:Activity:*")] // Activity admin (no resource access)
        [InlineData("GSFSGroup:*:*")] // Tenant admin
        ...
        [Theory]
        [InlineData("GSFSGroup:ResourceType:ResourceId")] // explicit resource grant
        [InlineData("*:ResourceType:ResourceId")] // resources shouldn't cross tenants.. so this probably a useless grant, but it's valid
        [InlineData("GSFSGroup:*:ResourceId")] // I sincerely hope you're not sharing ResourceIds across different aggregate resource types... but it's valid
        [InlineData("GSFSGroup:ResourceType:*")] // Resource admin for a single aggregate resource type
        [InlineData("GSFSGroup:*:*")] // Tenant admin
        [InlineData("*:*:*")] // System admin
        ...
        [InlineData("GSFSGroup:OtherResourceType:ResourceId")] // grant for a completely different kind of resource
        [InlineData("GSFSGroup:ResourceType:OtherResourceId")] // grant for a different resourceId
        ...
        [Theory]
        [InlineData("GSFSGroup:ActivityGroup:GenericGroup")] // explicit activity group grant
        [InlineData("*:ActivityGroup:GenericGroup")] // wildcard tenant
        [InlineData("GSFSGroup:*:GenericGroup")] // again with the strange but valid grant by type
        [InlineData("GSFSGroup:ActivityGroup:*")] // Any activity group checks out
        [InlineData("GSFSGroup:*:*")] // Tenant Admin
        [InlineData("*:*:*")] // System admin
        ...
        [InlineData("GSFSGroup:ActivityGroup:OtherGroup")] // grant for a different group
        [InlineData("Naisox:*:*")] // grant for wrong tenant
