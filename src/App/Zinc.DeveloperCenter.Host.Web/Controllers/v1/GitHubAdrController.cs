...
namespace Zinc.DeveloperCenter.Host.Web.Controllers.GitHub_API
{
    ...
    [Route("api/v{version:apiVersion}/{tenantId}/adrs")]
    public class GitHubAdrController : Controller
    {
        ...
        [ProducesResponseType(typeof(GitHubAdrModel), 200)]
        [ProducesResponseType(typeof(string), 400)]
        [ProducesResponseType(typeof(string), 401)]
        [ProducesResponseType(typeof(string), 403)]
        [ProducesResponseType(typeof(string), 500)]
        [ProducesResponseType(typeof(string), 501)]
        [HttpGet("{repoDotName}")] // need to query multiple pages, currently 2 as of 6/17/22
        public async Task<IActionResult> GitHubGetRepos(string repoDotName)
        {
