...
namespace RedLine.Application.A3.Authorization
{
    ...
    {
        private readonly AuthorizationPolicyFactory factory;
        ...
        public AuthorizationPolicyProvider(
            AuthorizationPolicyFactory factory,
        ...
        {
            var policy = await GetOpaPolicy(currentUser.UserId).ConfigureAwait(false);
            return new AuthorizationPolicy(policy, tenantId, currentUser.UserId);
        }

        private async Task<OpaPolicy> GetOpaPolicy(string userId)
        {
            ...
            policy.SetData(JsonSerializer.Serialize(authData, RedLineJsonSerializerOptions.Opa));
